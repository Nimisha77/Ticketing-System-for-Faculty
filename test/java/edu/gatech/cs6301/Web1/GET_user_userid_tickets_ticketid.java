...
public class GET_user_userid_tickets_ticketid {
    ...
    public void tsfsTest2() throws Exception {
        try {
            ...
            "Assignee2", new String[]{"User1"}, "2/28/23", "3/3/23");

            ticket2.close();

            CloseableHttpResponse isValid = getTicket("User2", idInt1);
    ...
    public void tsfsTest3() throws Exception {
        try {
            ...
            CloseableHttpResponse ticket1 = createTicket(
                userId, title, description, ticketStatus, category, emailAddress,
                assignee, watchers, createdDate, modifiedDate
            );
            String ticketid = getIdFromStringResponse(EntityUtils.toString(ticket1.getEntity()));
            ticket1.close();
            int ticketidInt = Integer.parseInt(ticketid);
            CloseableHttpResponse isValid = getTicket("User1", ticketidInt);
            int status = isValid.getStatusLine().getStatusCode();
            ...
            JSONAssert.assertEquals(expected, EntityUtils.toString(isValid.getEntity()), false);
        } catch (Exception e) {
            Assert.fail();
        } finally {
            httpclient.close();
